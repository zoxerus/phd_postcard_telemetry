...
control MyIngress (
    ...
    // variable.
    //
    // register_pointer stores the location of the last compartment that we
    // have written to in this switch and for this flow.
    register <bit<8>>  (totalSize) register_pointer;
    ...
    apply {
        if ( hdr.ipv4.isValid() ){
            ...
            if( local_metadata.is_postcard == 1){
                ...
                register_pointer.read(cursor, pointer);
                ...
                // for register_pointer.
                register_pointer.write(pointer, cursor);
