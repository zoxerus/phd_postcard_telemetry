...
control MyEgress(inout headers_t hdr,
                 inout local_metadata_t local_metadata,
                 inout standard_metadata_t standard_metadata) {
    register <bit<22>> (1) register_ipfix_sequence;
    bit<22> seq_num = 1;
    apply {
        ...
        if (IS_E2E_CLONE( standard_metadata ) ) {
            ...
            if ( ( local_metadata.mark_to_clone == 1 ) ){
                ...
                local_metadata.ingress_interface = standard_metadata.ingress_port;
                local_metadata.egress_interface = standard_metadata.egress_port;


            //    hash(local_metadata.flow_id, HashAlgorithm.crc16, (bit<32>)0,{
            //       hdr.ipv4.src_addr, hdr.ipv4.dst_addr, hdr.udp.src_port,
